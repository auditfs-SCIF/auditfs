...
fn test_full_scan_diff_cycle() {
    ...
    fs::write(
        dir.path().join("fichier.txt"),
        b"contenu modifie par attaquant",
    )
    .unwrap();
    ...
    assert!(
        ...
        "La modification doit être détectée"
    );
}
...
fn test_full_scan_no_changes() {
    ...
    let real_changes: Vec<_> = diff
        ...
        .filter(|c| {
            !matches!(
                c.change,
                auditfs::diff::compare::ChangeType::DangerousPermission
            )
        })
        .collect();
    assert!(
        real_changes.is_empty(),
        "Aucun changement si rien n'a été modifié"
    );
}
...
fn test_detect_new_file_added() {
    ...
    assert!(
        diff.changes
            .iter()
            .any(|c| matches!(c.change, auditfs::diff::compare::ChangeType::Added)),
...
fn test_daemon_detects_change() {
    ...
    assert!(
        !log.is_empty(),
        "Le fichier de log doit contenir une alerte"
    );
    assert!(
        log.contains("MODIFIÉ") || log.contains("config.txt"),
...
fn test_daemon_whitelist_filters_changes() {
    ...
    assert!(
        log.is_empty(),
        "Le log doit être vide si le fichier est dans la whitelist"
    );
}
