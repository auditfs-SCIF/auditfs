...
//         });
//     }
//     DirectorySnapshot { root: "/test".to_string(), files: map, created_at: 0 }
// }
...
//     assert!(
//         diff.changes.iter().any(|c| matches!(c.change, ChangeType::Added)),
...
//     let diff = compare(&before, &after);
//     assert!(
//         diff.changes.iter().any(|c| matches!(c.change, ChangeType::Removed)),
...
//     let diff = compare(&before, &after);
//     assert!(
//         diff.changes.iter().any(|c| matches!(c.change, ChangeType::Modified { .. })),
...
//         .collect();
//     assert!(real_changes.is_empty(), "Aucun changement si les snapshots sont identiques");
// }

// #[test]
// fn test_detect_dangerous_permission() {
//     let mut map = HashMap::new();
//     map.insert("/test/danger.sh".to_string(), FileSnapshot {
...
//     let after  = DirectorySnapshot { root: "/test".to_string(), files: map, created_at: 0 };
//     let diff = compare(&before, &after);
//     assert!(
//         diff.changes.iter().any(|c| matches!(c.change, ChangeType::DangerousPermission)),
...
//     let diff = DiffResult {
//         changes: vec![
//             FileChange { path: "/test/f.txt".to_string(), change: ChangeType::Added },
//             FileChange { path: "/test/g.txt".to_string(), change: ChangeType::Removed },
//         ],
//     };
...
//     let diff = DiffResult {
//         changes: vec![
//             FileChange { path: "/test/nouveau.txt".to_string(), change: ChangeType::Added },
//         ],
//     };
//     let text = report::to_text(&diff);
//     assert!(text.contains("AJOUTÉ"), "Le rapport texte doit mentionner AJOUTÉ");
...
//                 change: ChangeType::Modified { changed_attributes: vec!["sha256".to_string()] },
//             },
//         ],
//     };
//     let html = report::to_html(&diff);
//     assert!(html.contains("<table>"),          "HTML doit contenir une table");
//     assert!(html.contains("/test/modifie.txt"),"HTML doit contenir le chemin du fichier");
