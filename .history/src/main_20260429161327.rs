...
fn main() -> anyhow::Result<()> {
    ...
    match args[1].as_str() {
        "scan" => {
            ...
            // Appel à la fonction save (dans database/mod.rs ou database/serialize.rs)
            database::save(&db, &args[3], &args[4])?;
        ...
        "diff" => {
            ...
            let db = database::load(&args[2], &args[3])?;
        ...
        "daemon" => {
            ...
            loop {
                // Appel à la fonction run_once (définie dans src/daemon/scheduler.rs)
