...
fn main() -> anyhow::Result<()> {
    ...
    match args[1].as_str() {
        "scan" => {
            ...
            // Appel correct à la fonction `save` (publique)
            crate::database::serialize::save(&db, &args[3], &args[4])?;
        ...
        "diff" => {
            ...
            // Appel correct à `load`
            let db = crate::database::serialize::load(&args[2], &args[3])?;
        ...
        "daemon" => {
            ...
            // Boucle simple car la fonction `run_once` n'exécute qu'un seul scan.
            // Si vous avez une fonction `run` qui boucle déjà, remplacez par son appel.
            loop {
                // Nom de fonction : `run_once` (définie dans `src/daemon/scheduler.rs`)
                crate::daemon::scheduler::run_once(&args[2], &args[3], &args[4], &args[5], &wl)?;
