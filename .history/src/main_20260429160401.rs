...
fn main() -> anyhow::Result<()> {
    ...
    match args[1].as_str() {
        "scan" => {
            if args.len() < 5 { println!("Usage: auditfs scan <dossier> <baseline.db> <motdepasse>"); return Ok(()); }
            let snap = scanner::walker::scan(&args[2])?;
            let db = hashing::IntegrityDB { baseline: snap, version: "1.0".to_string() };
            database::serialize::save(&db, &args[3], &args[4])?;
            println!("Baseline sauvegardée dans {}", args[3]);
        }
        "diff" => {
            if args.len() < 4 { println!("Usage: auditfs diff <baseline.db> <motdepasse> [text|json|html]"); return Ok(()); }
        ...
        "daemon" => {
            if args.len() < 7 { println!("Usage: auditfs daemon <dossier> <baseline.db> <motdepasse> <interval> <log>"); return Ok(()); }
            let interval: u64 = args[5].parse().unwrap_or(3600);
            let wl = daemon::whitelist::Whitelist::new(vec![]);
            println!("Daemon démarré (interval={}s) — Ctrl+C pour arrêter", interval);
