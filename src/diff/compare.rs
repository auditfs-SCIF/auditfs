    ...
    for (path, after_snap) in &after.files {
        if let Some(before_snap) = before.files.get(path) {
            ...
            if before_snap.sha256 != after_snap.sha256 {
                attrs.push("sha256".to_string());
            }
            if before_snap.size != after_snap.size {
                attrs.push("size".to_string());
            }
            if before_snap.permissions != after_snap.permissions {
                attrs.push("permissions".to_string());
            }
            if before_snap.modified_at != after_snap.modified_at {
                attrs.push("modified_at".to_string());
            }
            if !attrs.is_empty() {
                changes.push(FileChange {
                    path: path.clone(),
            ...
            if after_snap.permissions & 0o002 != 0 {
                changes.push(FileChange {
                    path: path.clone(),
                    change: ChangeType::DangerousPermission,
                });
            }
        }
    }
