...
fn main() -> anyhow::Result<()> {
    ...
    match args[1].as_str() {
        "scan" => {
            ...
            println!("Scan réussi, création de la baseline...");
        ...
        "daemon" => {
            if args.len() < 6 {
                println!("Usage: auditfs daemon <dossier> <baseline.db> <motdepasse> <log.txt> [intervalle_sec]");
                return Ok(());
            }
            let interval = if args.len() >= 7 {
                args[6].parse().unwrap_or(60)
            } else {
                60
            };
